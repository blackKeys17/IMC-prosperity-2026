...
round_displayed = st.selectbox("Choose round:", [1, 2, 3])
...
if round_displayed == 1 or round_displayed == 2:
...
if round_displayed == 3:
    product = st.selectbox("Select product:", ["Hydrogel pack", "Velvetfruit extract", "Velvetfruit extract voucher"])
    day = st.selectbox("Select day:", [round_displayed-1, round_displayed-2, round_displayed-3])
    prices_dfs, trades_dfs = load_data(round=round_displayed)
    ...
    if product == "Hydrogel pack":
        prices_df = prices_dfs[-(day-round_displayed+1)]
        prices_df = prices_df[(prices_df["product"] == "HYDROGEL_PACK")].ffill()
        trades_df = trades_dfs[-(day-round_displayed+1)]
        trades_df = trades_df[trades_df["symbol"] == "HYDROGEL_PACK"]
    ...
    elif product == "Velvetfruit extract":
        prices_df = prices_dfs[-(day-round_displayed+1)]
        prices_df = prices_df[(prices_df["product"] == "VELVETFRUIT_EXTRACT")].ffill()
        trades_df = trades_dfs[-(day-round_displayed+1)]
        trades_df = trades_df[trades_df["symbol"] == "VELVETFRUIT_EXTRACT"]
    ...
    elif product == "Velvetfruit extract voucher":
        strike_price = st.selectbox("Strike price", [4000, 4500, 5000, 5100, 5200, 5300, 5400, 5500, 6000, 6500])
        prices_df = prices_dfs[-(day-round_displayed+1)]
        prices_df = prices_df[(prices_df["product"] == f"VEV_{strike_price}")].ffill()
        trades_df = trades_dfs[-(day-round_displayed+1)]
        trades_df = trades_df[trades_df["symbol"] == f"VEV_{strike_price}"]
